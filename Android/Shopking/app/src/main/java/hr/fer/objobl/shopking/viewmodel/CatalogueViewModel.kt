...
class CatalogueViewModel(
    ...
    val categories: MutableLiveData<List<CategoryViewState>> by lazy {
        MutableLiveData(
            categoryDataSource.categories.value?.mapToViewStateList()?.mapIndexed { index, categoryViewState ->
                if (index == 0) {
    ...
    fun selectCategory(category: String) {
        categories.value = categories.value?.map { CategoryViewState(it.id, it.name, it.name == category) }
    }
}
